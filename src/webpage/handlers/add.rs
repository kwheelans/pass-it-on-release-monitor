...
use maud::Markup;
use tracing::debug;
use crate::webpage::add_page::{add_github_monitor_page, add_rancher_channel_page};
use crate::webpage::handlers::ADD_RECORD_TITLE;
...
pub async fn get_ui_add_monitor(
    ...
    match monitor_type.as_str() {
        TYPE_NAME_GITHUB => {
            Ok(add_github_monitor_page(format!("{} - {}", ADD_RECORD_TITLE, "Github").as_str()).await)
        }
